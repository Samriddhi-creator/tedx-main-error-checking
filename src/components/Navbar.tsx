...
export default function Navbar() {
    ...
    return (
        <>
            ...
            <div 
                className={`fixed inset-0 w-screen h-screen bg-black/95 backdrop-blur-md z-[100] flex px-6 md:px-16 py-8 md:py-12 transition-all duration-500 ease-in-out ${
                    open 
            ...
            >
                ...
                <div 
                    className={`hidden md:flex flex-col justify-end gap-4 w-1/2 transition-all duration-700 ease-out transform ${
                        open ? "translate-x-0 opacity-100 delay-150" : "-translate-x-8 opacity-0"
                    }`} 
                ...
                <div 
                    className={`md:hidden absolute bottom-8 left-6 flex flex-col gap-3 transition-all duration-700 ease-out transform ${
                ...
                <button 
                    onClick={() => setOpen(false)} 
                    className={`absolute top-[9px] md:top-[13px] right-4 md:right-12 size-8 md:size-14 rounded-full border-2 border-white flex items-center justify-center text-white text-sm md:text-xl hover:bg-white/10 transition-all duration-500 transform ${
                        open ? "rotate-0 scale-100 opacity-100 delay-200" : "rotate-90 scale-75 opacity-0"
                    }`}
                >
                    ✕
                </button>
