...
export default function Navbar() {
    ...
    return (
        <>
            <header className="relative w-full h-[50px] md:h-[80px] flex items-center justify-between pr-4 md:pr-8 overflow-hidden">
                ...
                <div className="flex items-center gap-2 md:gap-4">
                    ...
                    <button
                        ...
                        className="size-8 md:size-14 bg-gradient-to-b from-red-600 to-red-900 rounded-full flex flex-col items-center justify-center gap-[3px] md:gap-[5px]">
                        <span className="w-3 md:w-5 h-[2px] md:h-[3px] bg-white rounded-full" />
                        <span className="w-3 md:w-5 h-[2px] md:h-[3px] bg-white rounded-full" />
                        <span className="w-3 md:w-5 h-[2px] md:h-[3px] bg-white rounded-full" />
                    </button>
                </div>
            ...
            {open && (
                <div className="fixed inset-0 w-screen h-screen bg-black z-[100] flex px-6 md:px-16 py-8 md:py-12" onClick={() => setOpen(false)}>

                    {/* Left side - desktop only */}
                    <div className="hidden md:flex flex-col justify-end gap-4 w-1/2" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="flex flex-col justify-start md:justify-center gap-0 w-full md:w-1/2 pt-12 md:pt-0" onClick={(e) => e.stopPropagation()}>
                        {navLinks.map(({ label, href }) => (
                            <a key={label} href={href}
                                ...
                                className={`text-5xl font-normal font-['Bebas_Neue'] py-2 md:py-3 border-b border-white/20 transition-all
                                    ${hovered === label
                                        ? "bg-gradient-to-b from-red-700 from-35% via-white via-50% to-red-400 to-65% bg-clip-text text-transparent"
                    ...
                    <div className="md:hidden absolute bottom-8 left-6 flex flex-col gap-3" onClick={(e) => e.stopPropagation()}>
